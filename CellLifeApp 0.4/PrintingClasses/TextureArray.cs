...
namespace CellLifeApp.PrintingClasses
{
    ...
    class TextureArray
    {
        ...
        public const int maxResolution = 9; // 2^(9-1) = 256 , максимальный размер текстурки будет 256*256 пикселей
        public const int precision = 20;
        internal static int range;
            ...
            foreach (string type in types)
            {
                ...
                for (int i = 0; i < maxResolution; i++)
                {
                    ...
                    Image GetGradientImage(Image image, int gradient)
                    {
                        ...
                        int a = gradient * range * 255 / Cell.parameters.ToxicLevel;
        ...
        private Image GetDefaultImage(Cell cell)
        {
            ...
            if (cell.Organic >= Cell.parameters.ToxicLevel && cell.Charge >= Cell.parameters.ToxicLevel)
                return SizedTextures["AllToxic"][Resolution];
            if (cell.Organic >= Cell.parameters.ToxicLevel)
                return SizedTextures["OrganicToxic"][Resolution];
            if (cell.Charge >= Cell.parameters.ToxicLevel)
        ...
        private Image GetModedImage(Cell cell)
        {
            ...
            if (cell.Organic >= Cell.parameters.ToxicLevel && cell.Charge >= Cell.parameters.ToxicLevel)
                return SizedTextures["AllToxic"][Resolution];
            if (CamMode != "Organic" && cell.Organic >= Cell.parameters.ToxicLevel)
                return SizedTextures["OrganicToxic"][Resolution];
            if (CamMode != "Charge" && cell.Charge >= Cell.parameters.ToxicLevel)
                return SizedTextures["ChargeToxic"][Resolution];
        ...
        private Image GetNoToxicsImage(Cell cell)
        {
            if (cell == null)
                return SizedTextures["NoTexture"][Resolution];
            if (!rotatebleTypes.Contains(cell.GetType().Name) && gradientableTypes.Contains(cell.GetType().Name))
